      ...
      <div className="row mt-5 border rounded shadow-sm pt-2 pb-2">
        <div className="col-12">
          <form className="form-inline">
            ...
            <button type="button" className="btn btn-success" onClick={this.onCreate}>Save</button>
          </form>
        </div>
      </div>
          ...
          <tbody>
            {
              this.state.people.map((x) => <tr key={x.id}>
                <td className="d-none">{x.id}</td>
                <td>{x.firstname}</td>
                <td>{x.lastname}</td>
                <td>{x.mail}</td>
                <td>{x.phone}</td>
                <td>
                  <button type="button" className="btn shadow-sm text-danger btn-sm" onClick={() => this.onDelete(x.id)}><FontAwesomeIcon icon={faTrash} /></button>
                </td>
              </tr>)
            }
          </tbody>
